...
import { fetchTagOptions } from '../../services/api/filterOptionsApi';
...
export function TagFilterModal({ open, selectedIds, onToggle, onClose }: Props) {
  const [query, setQuery] = useState('');
  const [options, setOptions] = useState<FilterOption[]>([]);

  useEffect(() => {
    if (!open) return;
    void fetchTagOptions(query).then(setOptions).catch(() => setOptions([]));
  }, [open, query]);
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <OptionList ariaLabel="tag-options" options={options} selectedIds={selectedIds} onToggle={onToggle} />
      </div>
    </ModalShell>
  );
}